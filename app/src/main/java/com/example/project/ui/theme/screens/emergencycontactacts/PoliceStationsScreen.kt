...
@Composable
fun PoliceStationsScreen(navController: NavHostController){
    Column(
        ...
        TopAppBar(title = { Text(text = "Hospitals PhoneBook", color = Color.White) },
            ...
            navigationIcon = {
                IconButton(onClick = {
                    navController.navigate(ADDFIRSTAID_URL)
                })
                {
                    Icon(
...
fun PoliceStationsScreenPreview(){
    ResQTheme {
        PoliceStationsScreen(navController = rememberNavController())
    }
}